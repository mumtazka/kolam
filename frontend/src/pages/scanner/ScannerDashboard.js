...
const ScannerDashboard = () => {
  ...
  const handleScan = async (ticketId) => {
    try {
      const response = await axios.post(`${API}/tickets/scan`, { ticket_id: ticketId });
  ...
  const getResultColor = () => {
    ...
    if (scanResult.status === 'VALID') return 'scan-result-valid';
    if (scanResult.status === 'USED') return 'scan-result-used';
    return 'scan-result-invalid';
  };
    ...
    <div className="min-h-screen bg-slate-50" data-testid="scanner-dashboard">
      ...
      <header className="bg-white border-b border-slate-200 px-4 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900" style={{fontFamily: 'Outfit'}}>AquaFlow Scanner</h1>
      ...
      <div className="p-4 max-w-2xl mx-auto">
        {/* Scan Result */}
        {scanResult && (
          <div className={`${getResultColor()} text-white rounded-xl p-8 mb-6 text-center`} data-testid="scan-result">
            ...
            {scanResult.ticket && (
              <Card className="bg-white/20 backdrop-blur-sm p-4 text-left">
                <div className="space-y-2 text-sm">
                  <p><span className="font-semibold">Category:</span> {scanResult.ticket.category_name}</p>
                  <p><span className="font-semibold">Price:</span> Rp {scanResult.ticket.price?.toLocaleString()}</p>
                  <p><span className="font-semibold">Created:</span> {new Date(scanResult.ticket.created_at).toLocaleString()}</p>
                  {scanResult.ticket.nim && <p><span className="font-semibold">NIM:</span> {scanResult.ticket.nim}</p>}
                </div>
              </Card>
            )}
            <Button
              ...
              {!scanning ? (
                <div className="text-center py-12">
                  ...
                  <Button
                    ...
                    className="bg-slate-900 hover:bg-slate-800 h-14 px-8 text-lg"
