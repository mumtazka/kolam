...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats({
        totalStaff: usersRes.data.filter(u => u.is_active).length,
        todayTickets: reportRes.data.tickets_sold,
        todayRevenue: reportRes.data.total_revenue,
        todayScans: reportRes.data.tickets_scanned
      });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Button className="h-20 bg-slate-900 hover:bg-slate-800" data-testid="view-reports-button">
            View Today's Report
          </Button>
          <Button variant="outline" className="h-20" data-testid="manage-staff-button">
            Manage Staff
          </Button>
          <Button variant="outline" className="h-20" data-testid="update-prices-button">
            Update Prices
          </Button>
        </div>
      </Card>
